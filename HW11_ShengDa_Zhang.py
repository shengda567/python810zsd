...
class Repository:
        ...
        def __init__(self, path, ptable=False):
            ...
            if ptable:
                ...
                self.instructor_prettytable2(self.instructor_table_db(os.path.join(path, "810_startup.db")))
        ...
        def instructor_table_db(self, db_path):
            ...
            data = []
            for row in db.execute("select instructors.CWID, instructors.Name, instructors.Dept, grades.Course, count(*) from instructors join grades on instructors.CWID=grades.InstructorCWID group by Course"):
                data.append(row)
            return data

        def instructor_prettytable2(self, data):
            pt = PrettyTable(field_names=Instructor.PT_FIELDS)
            for line in data:
                pt.add_row(line)
            print(pt)
...
class Major:
    ...
    def remaining_electives(self, course_grades):
        ...
        if len(rem_electives) < len(self._electives):
            rem_electives = None
...
def main():
    stevens = Repository("E:\sww810\homework11", ptable=True)
